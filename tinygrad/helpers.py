...
def ansicolor(color:str, background=False): return f"\u001b[{10*background+60*(color.upper() == color)+30+['black', 'red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'white' ].index(color.lower())}m" # noqa: E501
def colored(st, color:Optional[str], background=False): return f"{ansicolor(color, background)}{st}\u001b[0m" if color is not None else st  # replace the termcolor library with 2 lines # noqa: E501
